...
def insert_data(name, email, res_score, timestamp, no_of_pages, reco_field, cand_level, skills, recommended_skills,
                courses, education, work_experience):
...
def extract_work_experience(text):
    # Regular expression to find work experience information
    experience_pattern = r'(?i)work\s*experience\s*([\s\S]*?)(?=\n\n|\n[A-Z]|$)'
    matches = re.findall(experience_pattern, text)

    work_experience_list = []

    for match in matches:
        # Add the entire work experience block to the list
        work_experience_list.append(match.strip())
        ...
        if resume_data:
            try:
                if len(resume_data) >= 8:
                    _, name, email, resume_score, timestamp, page_no, reco_field, cand_level, skills, recommended_skills, recommended_courses, education, work_experience = resume_data
    ...
    if choice == 'Chatbot':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                # Display and store experience information
                if experience:
               ...
               # st.subheader("**Resume Tips & Ideas💡**")
                ...
                insert_data(resume_data['name'], resume_data['email'], str(resume_score), timestamp,
                    ...
                    str(resume_data['skills']), str(recommended_skills), str(rec_course), str(education), str(work_experience))
        ...
        if st.button('Login'):
            if ad_user == 'crossroad' and ad_password == 'abc123':
                ...
                df = pd.DataFrame(data, columns=['ID', 'Name', 'Email', 'Resume Score', 'Timestamp', 'Total Page',
                                                 ...
                                                 'Recommended Course', 'education', work_experience])
                st.dataframe(df)
